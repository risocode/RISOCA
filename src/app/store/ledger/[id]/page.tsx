...
export default function CustomerLedgerPage() {
  ...
  const outstandingCreditTransactions = useMemo(() => {
    return transactions
      .filter(
        (tx) =>
          tx.type === 'credit' && !paidCreditIds.has(tx.id) && tx.status !== 'deleted'
      )
      .sort(
    ...
    if (data.type === 'credit') {
      if (creditMode === 'item') {
        ...
        if (!data.description?.trim()) {
         ...
         if (data.amount <= 0) {
           toast({ variant: 'destructive', title: 'Error', description: 'Amount must be positive for cash credit.' });
           setIsSubmitting(false);
           return;
        }
        payload = {
      ...
      payload = {
        ...
        paidCreditIds: selectedCredits.size > 0 ? Array.from(selectedCredits) : undefined,
      };
    }
                  ...
                  >
                    ...
                    <TabsContent value="credit" className="pt-4">
                      <Tabs
                        ...
                        value={creditMode}
                        onValueChange={(v) =>
                          setCreditMode(v as 'item' | 'cash')
                        }
                      >
                        <TabsList className="grid w-full grid-cols-2">
                          <TabsTrigger value="item">Add by Item</TabsTrigger>
                          <TabsTrigger value="cash">Add by Cash</TabsTrigger>
                        </TabsList>
