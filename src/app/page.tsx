...
import { Upload, ReceiptText, Bot, X, FileImage, ServerCrash } from 'lucide-react';
import { diagnoseReceipt, type DiagnoseReceiptOutput } from '@/ai/flows/diagnose-receipt-flow';
...
export default function Home() {
  ...
  const [diagnosis, setDiagnosis] = useState<DiagnoseReceiptOutput | null>(null);
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // For preview, we use a blob URL which is more memory efficient
        setImagePreview(URL.createObjectURL(file));
        // For sending to the AI, we use the base64 data URI
  ...
  const handleScanReceipt = async () => {
    ...
    try {
      ...
      console.error(e);
      setError('The AI could not process the receipt. It might be blurry or unsupported. Please try another image.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderInitialState = () => (
    <Card className="w-full max-w-lg shadow-2xl shadow-primary/10">
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Bot /> AI Receipt Scanner</CardTitle>
  ...
  const renderPreviewState = () => (
    <Card className="w-full max-w-lg shadow-2xl shadow-primary/10">
      <CardHeader>
        ...
        <CardDescription>Click the button below to analyze your receipt.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="relative w-full overflow-hidden rounded-lg aspect-[16/10] border">
          {imagePreview && <Image src={imagePreview} alt="Receipt preview" layout="fill" objectFit="contain" />}
        </div>
  ...
  const renderLoadingState = () => (
     <Card className="w-full max-w-2xl shadow-2xl shadow-primary/10">
      <CardHeader>
        ...
        <CardDescription>The AI is working its magic. Please wait...</CardDescription>
      </CardHeader>
  ...
  const renderResultsState = () => diagnosis && (
    <Card className="w-full max-w-3xl shadow-2xl shadow-primary/10">
       <CardHeader>
        ...
        <CardDescription>Here's what the AI found on your receipt.</CardDescription>
      </CardHeader>
       <CardContent className="grid gap-8 md:grid-cols-2">
          <div className="relative w-full overflow-hidden rounded-lg aspect-[16/10] border">
            {imagePreview && <Image src={imagePreview} alt="Receipt" layout="fill" objectFit="contain" />}
          </div>
          <div className="space-y-4">
            ...
            <div>
              ...
              <p>{new Date(diagnosis.transactionDate + 'T00:00:00').toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</p>
            </div>
             <div>
              <p className="text-sm font-medium text-muted-foreground">Total</p>
              <p className="text-2xl font-bold text-primary">${diagnosis.total.toFixed(2)}</p>
            </div>
             <div>
              <p className="text-sm font-medium text-muted-foreground">Items ({diagnosis.items.length})</p>
              <div className="p-3 border rounded-md max-h-48 overflow-y-auto text-sm bg-muted/50">
                <ul className="space-y-2">
                  {diagnosis.items.map((item, index) => (
                    <li key={index} className="flex justify-between py-1 border-b last:border-none">
       ...
       <div className="p-6 pt-0">
          <Button onClick={handleReset} className="w-full"><ReceiptText className="mr-2" /> Scan Another Receipt</Button>
       </div>
    </Card>
  );
  ...
  const renderErrorState = () => (
     <Card className="w-full max-w-lg shadow-2xl shadow-destructive/20">
      <CardHeader>
        ...
        <CardDescription>Sorry, we couldn't process your receipt.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Alert variant="destructive">
            <ServerCrash className="w-4 h-4" />
        ...
        <div className="relative w-full overflow-hidden rounded-lg aspect-video opacity-50 border">
            {imagePreview && <Image src={imagePreview} alt="Receipt with error" layout="fill" objectFit="contain" />}
        </div>
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-4 bg-background">
      ...
      {isLoading ? renderLoadingState() :
       diagnosis ? renderResultsState() :
       error && imagePreview ? renderErrorState() :
       imagePreview ? renderPreviewState() :
       renderInitialState()
      }
    </main>
  );
